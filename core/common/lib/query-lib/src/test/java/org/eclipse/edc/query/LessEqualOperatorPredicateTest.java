...
class LessEqualOperatorPredicateTest {

    private final OperatorPredicate predicate = new LessEqualOperatorPredicate();

    @ParameterizedTest
    @ArgumentsSource(ValidValues.class)
    void shouldReturnTrue_whenValueLessThanOrEqualToComparedOne(Object value, Object comparedTo) {
        assertThat(predicate.test(value, comparedTo)).isTrue();
    }

    @ParameterizedTest
    @ArgumentsSource(InvalidValues.class)
    void shouldReturnFalse_whenValueNotLessThanOrEqualToComparedOne(Object value, Object comparedTo) {
