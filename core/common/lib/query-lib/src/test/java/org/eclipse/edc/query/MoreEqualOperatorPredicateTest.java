...
class MoreEqualOperatorPredicateTest {

    private final OperatorPredicate predicate = new MoreEqualOperatorPredicate();

    @ParameterizedTest
    @ArgumentsSource(ValidValues.class)
    void shouldReturnTrue_whenValueMoreThanOrEqualToComparedOne(Object value, Object comparedTo) {
        assertThat(predicate.test(value, comparedTo)).isTrue();
    }

    @ParameterizedTest
    @ArgumentsSource(InvalidValues.class)
    void shouldReturnFalse_whenValueNotMoreThanOrEqualToComparedOne(Object value, Object comparedTo) {
